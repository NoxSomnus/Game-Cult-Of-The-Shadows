...
public class CharacterController2DCoyoteTime : MonoBehaviour
{
	...
	public void Move(float move, bool crouch, bool jump)
	{
        ...
        if ((m_Grounded || m_CoyoteTimeCounter > 0f) && jump)
        {
            ...
            Debug.Log($"Jump force applied: {m_JumpForce}");
        }
        else if (jump)
        {
            Debug.Log($"Jump failed. m_Grounded: {m_Grounded}, m_CoyoteTimeCounter: {m_CoyoteTimeCounter}");
        }

        // Log the velocity after attempting to jump
        Debug.Log($"Velocity after jump attempt: {m_Rigidbody2D.velocity}");
